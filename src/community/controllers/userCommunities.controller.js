...
class UserCommunitiesController {
    ...
    async getMyUserCommunities(req, res, next) {
        try {
            ...
            // Get communities using the user ID
            const [communities, total] = await Promise.all([
                userCommunitiesModel.getUserCommunities(userId, options),
                userCommunitiesModel.countUserCommunities(userId, {
                    search: options.search,
                }),
            ]);
