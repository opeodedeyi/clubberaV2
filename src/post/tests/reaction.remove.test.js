// src/post/tests/reaction.remove.test.js
...
describe("Remove Post Reaction", () => {
    ...
    beforeEach(() => {
        ...
        PostReactionModel.removeReaction.mockResolvedValue({
            id: 1,
            post_id: 1,
            user_id: 1,
            reaction_type: "like",
    ...
    it("should remove a reaction successfully", async () => {
        await PostReactionController.removeReaction(req, res, next);

        expect(PostModel.findById).toHaveBeenCalledWith("1");
        expect(PostReactionModel.removeReaction).toHaveBeenCalledWith(
            "1",
            1,
            "like"
        );
    ...
    it("should return 404 when reaction not found", async () => {
        PostReactionModel.removeReaction.mockResolvedValue(null);

        await PostReactionController.removeReaction(req, res, next);

        expect(next).toHaveBeenCalled();
        expect(ApiError).toHaveBeenCalledWith("Reaction not found", 404);
    });
